...
filename = 'tester.csv' # example: 'adult.data' (with full path if not in same folder)
separation = '\t'
...
K = 2 #Meta-parameter for evaluation of encoders
for model in Models:
    model_dict = {}
...
pd.DataFrame(table, index=Models.keys(), columns=Encoders.keys()).to_latex('table_'+name_prefix+'.tex')
